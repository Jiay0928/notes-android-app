...
class Model {
    ...
    fun addSomeNotes() {
        ...
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Seven Note", "This is the oldest, yet, IMPORTANT (!) note", important = true, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Eight Note","This is an archived note", important = false, archived = true))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Nine Note","This is a pretty boring note; not a lot to see here...", important = false, archived = false))
        Thread.sleep(10L)
        notes.add(ModelNote(generateID(), "Ten Note","This is a note with a reasonably long content / body. Yep, there are quite a few characters in this text; enough so that this note has to be displayed over multiple lines on the Edit-screen.", important = false, archived = false))
        Thread.sleep(10L)
    }
}